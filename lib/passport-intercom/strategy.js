...
function Strategy(options, verify) {
  ...
  if(options.requireVerifiedEmail !== undefined) {
    this.requireVerifiedEmail = options.requireVerifiedEmail;
  } else {
    this.requireVerifiedEmail = true;
  }
}
...
Strategy.prototype.userProfile = function(accessToken, done) {
  ...
  this._oauth2.get('https://api.intercom.io/me', encodedToken, function (err, body, res) {
    ...
    if (this.requireVerifiedEmail && json.email_verified != true) {
